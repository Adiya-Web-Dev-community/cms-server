...
const {addDropdownMenu,Delet_DropDown_By_Id,Update_Nav_By_Id,Update_Comonent,Update_LayoutType,Update_Layout_data}=require("./socketHelper/index");
const app = express();
...
app.use(require("./router/templateRouter"));
    ...
    io.on('connection', (socket) => {
        ...
        socket.on("UpdateNav",async(item)=>{
            ...
            try {
                ...
                // console.log("Nav Updated Item>>>",item);
                if(UserId){
                    const res=await Update_Nav_By_Id(item?.data,item?.id,item.productId);
        ...
        socket.on("setLayout_type",async(item)=>{
            try {
                ...
                if(UserId){
                    const res=await Update_LayoutType(item?.data,item.productId,item?.navId,item?.path);
        ...
        socket.on("Update_Layout_By_id",async(item)=>{
            try {
                ...
                if(UserId){
                    const res=await Update_Layout_data(item?.data,item.productId,item?.navId,item?.layoutId);

...
const layout = require("../models/web/layout");
...
const Delet_DropDown_By_Id = async (id, productId) => {
  try {
    ...
    console.log(
      "data navbar????????>>>>",
      isProduct?.navigationArr?.filter((item) => item?._id.toString() === id)
    );
    if (!isProduct) {
      return data({
        suucess: false,
    ...
    isProduct.navigationArr = isProduct.navigationArr.filter(
...
const Update_Nav_By_Id = async (data, id, productId) => {
  try {
    ...
    const indexToUpdate = product.navigationArr.findIndex(
      (item) => item?._id.toString() === id
    );
